    ...
    try {
        ...
        if (fs.existsSync(DATA_PATH)) {
            ...
            if (!stopsData.stops || !Array.isArray(stopsData.stops)) {
                ...
                console.log(`Sample stops data: ${JSON.stringify(stopsData.stops.slice(0, 5))}`);
            }
        }
    } catch (e) {
...
app.get('/api/stops/nearby', (req, res) => {
    try {
        ...
        // Проверка, что lat и lon являются числами
        lat = parseFloat(lat);
        lon = parseFloat(lon);
        ...
        if (!Array.isArray(stopsData.stops)) {
            console.error('stopsData.stops is not an array');
            return res.status(500).json({
                success: false,
                error: 'Stops data is not an array'
            });
        }

        // Filter out undefined elements from stopsData.stops
        ...
        const filteredStops = stopsData.stops.filter(stop => {
            ...
            console.log(`Stop: ${stop.stop_id}, Lat: ${stopLat}, Lon: ${stopLon}, Distance: ${dist}`);
            return dist <= distance;
        });

        console.log(`Returning ${filteredStops.length} stops for lat=${lat}, lon=${lon}, distance=${distance}`);
        res.json({
            success: true,
            stops: filteredStops,
            source: 'local',
...
app.get('/api/stops/:stopId/schedule', async (req, res) => {
    ...
    const scheduleInterval = 90; // Увеличиваем интервал до 90 минут
    ...
    try {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const now = new Date();
                const end = new Date(now.getTime() + scheduleInterval * 60 * 1000); // Используем увеличенный интервал
                ...
                const response = await fetch(url, {
                    timeout: 10000,
                    headers: {
                        'User-Agent': 'WinnipegTransitApp/1.0'
                    }
                });
                ...
                const data = await response.json();
                console.log(`Schedule data for stop ${stopId}:`, JSON.stringify(data));
                const result = {
                    success: true,
                    data
                };
...
app.listen(port, () => {
    ...
    if (isUpdatedToday()) {
        console.log('Data already updated today, skipping fetch. Next update at 3:00 AM.');
    } else {
