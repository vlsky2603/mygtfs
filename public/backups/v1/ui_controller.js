...
function toggleDarkMode() {
    ...
    const iosBtn = document.getElementById('ios-theme-button');
    if (iosBtn) iosBtn.innerHTML = darkMode ? '<i class="fas fa-sun"></i>' : '<i class="fas fa-moon"></i>';
...
function updateResetRouteButtonVisibility() {
    ...
    const iosClearButton = document.getElementById('ios-clear-filter-button');
    if (iosClearButton) {
        if (filters.route || isLiveBusViewActive) {
            iosClearButton.classList.remove('hidden');
        } else {
            iosClearButton.classList.add('hidden');
        }
    }

    // Hide radius circle when in route view or live bus view
    if (radiusCircle) {
...
function updateBodyPanelOpenClass() {
    ...
    syncDockButtons(activePanel?.id || null);
}

function syncDockButtons(activePanelId) {
    const dockButtons = document.querySelectorAll('.ios-dock-button[data-panel-target]');
    dockButtons.forEach(button => {
        button.classList.toggle('active', !!activePanelId && button.dataset.panelTarget === activePanelId);
    });
}
